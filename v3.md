...
- [Search Engine Feedback Endpoint](#search-engine-feedback-endpoint)
- [Examples](#examples)
  - [node.js](#nodejs)
  - [PHP](#php)
...
## Notable changes compared to v2
* Lookup endpoint added: find artists by their instagram or twitter username
* Integration endpoint added: notify artist's management team that you've integrated them into your system
* Muzooka search engine feedback endpoint added: if you encounter missing or incoherent results from our search engine - send them for investigation!  
* Search endpoint now returns minimal set of data and is much faster
* Alias artist profiles are properly supported now
...
To find an artist by one of it's handles in social networks and services, use Artist Lookup Endpoint.
...
You can only provide ONE of the above parameters at a time.
...
```bash
curl -X GET \
  'https://devapi.muzooka.com/v3/artists?instagram=taylorswift' \
  -H 'x-api-key: REPLACE_THIS_WITH_YOUR_API_KEY'
```

</p></details>

Response will have exactly the same format as [Artist Endpoint](#artist-endpoint)

## Search Endpoint

To search for an artist, use their name or a portion of it for a fuzzy match. Use following endpoint:

`GET` `https://devapi.muzooka.com/v3/artists/search`
...
`POST` `https://devapi.muzooka.com/v3/feedback/integrations`
...
## Search Engine Feedback Endpoint

If you use Muzooka search engine to make initial connections between your internal artist records and Muzooka Artists Pages, some artists can either be missing or yield incoherent results.

To notify Muzooka team about such search outcomes, use the endpoint:

`POST` `https://devapi.muzooka.com/v3/feedback/searches`

Send following parameters in your request body, as JSON:

- `searchRequestId`: Value of `X-Muzooka-SearchId` header you got back from search engine after processing your request
- `comment`: Human-readable comment that could be useful on manual review of search results 
